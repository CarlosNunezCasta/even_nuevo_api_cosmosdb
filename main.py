...
@app.get("/event/{event_id}", response_model=Evento)
...
def update_event(event_id:str, updated_event: Evento):
    ...
    raise HTTPException(status_code=400, detail='No se pudo insetar, la cantidad es menor que la cantidad de participantes')
...
def add_participant(event_id: str, participant:Participante):
    # Obtenemos el evento
    # Validar que la cantidad de participantes en el evento sea mayor o igual que la capacidad y de error
    # Validar si el participante ya existe
    try:
        event=container.read_item(item=event_id, partition_key=event_id)
        if len(event['participants']) >= event['capacity'] :
            raise HTTPException(status_code=400, detail='Capacidad maxima del evento alcanzada')
